function SummarizeParametricConditions1(subjectToAnalyze)
% SummarizeParametricConditions1(subjectToAnalyze)
%
...
for s = subjectToAnalyze
    ...
    for c = 1:length(summaryDataStruct)
        ...
        if (summaryDataStruct{c}.test.brightLight ~= expectedTestBrightLight)
            error('Unexpected reference center value');
        end
        ...
        % consistency for additional subjecgts
        if (s == 1)
...
if (length(subjectToAnalyze) > 1)
    ...
    fprintf(fid,'This table created by SummarizeParametricConditions1.m. ');
    ...
    fprintf(fid,'  * Test bright illum value is %d\n',expectedTestBrightLight);
    fprintf(fid,'  * Test nChecks value is %d\n',expectedTestnChecks);
    
    fprintf(fid,'\n');
    fprintf(fid,'<WRAP leftalign>\n');
    fprintf(fid,'^ Condition Num ^ Test Type ^ Ref Mean ^ Test Mean ^ Ref Probe ^ Test Probe ^ Test Blk ^ Test Cen ^\n');
    for c = 1:length(summaryDataStruct)
