function makeTrialStructStdFirstImage(varargin)
%%makeTrialStructStdFirstImage Make a struct with trial information about one experiment
%
% This is same as makeTrialStruct, but the standard image is chosen to be
% the one which was rendered with the first reflectance index. This
% function is mainly going to be used for Experiment 4, where we present
% the standard image with the target at the center and the comparison image
% with target at other places.
%
% Usage: 
%   makeTrialStructStdFirstImage();
%
% Description:
...
%     for iterCmpLevels = 1 : length()
    ...
    % comparison levels
    tempStdIndex = ones(1,nCmpLevels);
    ...
    for ii = 1 : length(tempCmpLvl)
        ...
        if (parser.Results.comparisionTargetSameSpectralShape)
            ...
            trialStruct.trialCmpIndex((iterBlocks-1)*nCmpLevels+ii) = indexOfCmpImages(1+randi(length(indexOfCmpImages)-1));
        end
    end
